...
def export(shellname, strings):
    ...
    shellname = re.sub('[^0-9a-zA-Z]+', '_', shellname)
...
def export_from_webshell():
    ...
    if options.filename != None:
        ...
        export("DATASET-{}".format(options.filename), strings)
        ...
        for dirName, subdirList, fileList in os.walk(rootDir):            
            for fname in fileList:
                ...
                export("DATASET_{}".format(filename), strings)
...
def export_from_rawlist():
    try:
        with open('res/rawlist.txt') as f:
            lines = f.readlines()

            for line in lines:
                line = line.strip()
                if line != '':
                    name = 'RAWLIST_{}'.format(line)
                    export(name, [line])

    except IOError, e:
        print red("[+] Can't open res/rawlist.txt => Unable to export fingerprint from raw list: " + str(e))
        return False
...
def export_from_pmf():
    try:
        ...
        print red("[+] Can't open res/pmf.yara => Unable to export patterns from pmf content: " + str(e))
        return False   
    return True
...
def export_adware_filter():
    try:
        ...
        print red("[+] Can't open res/adware.yara => Unable to export patterns from adware content: " + str(e))
        return False   
    return True
